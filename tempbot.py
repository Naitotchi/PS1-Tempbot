...
def ask_for_temp(**payload):
    data = payload['data']
    web_client = payload['web_client']

    # We are listening for the "!temp" command, which we'll then 
    # use to see if they want everything, or a particular area,
    # or nothing, in which case we'll show the help
    if '!temp' in data['text'].lower():
        # Send the text to the fuction and get the response back which
        # is what we'll tell the person
        response = build_response(data['text'].lower())
...
# Now let's set up the Slack part
print("Starting the slack thread...")
rtm_client = RTMClient(token=config['Slack']['token'])
rtm_client.start()
